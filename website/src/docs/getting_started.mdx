...
You can look at the [contents of the script](https://garn.io/install.sh).
...
1. **Install Nix**. Follow the [instructions to install Nix](https://nixos.org/download).
...
<Garn /> provides an `edit` command that downloads and starts a VSCodium configured with Deno LSP. It is started with its own configuration and data directories, so as to not clobber any VSCodium or VSCode configuration you may already have.
